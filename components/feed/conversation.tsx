...
/**
 * Post-page conversation with a thin reply graph (hairline + border-l nest).
 */
...
function Node({
  ...
  return (
    <div id={`comment-${comment.id}`} className="relative flex gap-2.5">
        ...
        {hasReplies ? (
          atLimit && !showDeeper ? (
            <button
              ...
              className="mt-1.5 text-[12.5px] text-brand transition hover:underline"
            >
              Continue this thread ({hidden} more)
            </button>
          ) : (
            <ul className="mt-2 space-y-3 border-l border-foreground/15 pl-3">
